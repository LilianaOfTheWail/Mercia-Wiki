...
export async function buildTableOfContents(
  { domain, filters, dataFields }: DataViewQueryInput
): Promise<Array<MetadataSummaryProps>> {
  const dataSource = await fetch(buildPublicUrl(`data-config/${getDataFileName(domain)}`))

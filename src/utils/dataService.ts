...
async function loadCategory(category: string): Promise<NavigationDataItem[]> {
  ...
  try {
    const response = await fetch(buildPublicUrl(`data-config/${getDataFileName(sourceKey)}`))
    if (!response.ok) {
      return []
    }
...
function findInNestedList(list: NavigationDataItem[] | undefined, targetKey: string): NavigationDataItem | null {
...
export async function getCategoryItems(category: string): Promise<NavigationDataItem[]> {

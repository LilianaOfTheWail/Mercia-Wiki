...
interface NestedNavItemProps {
  ...
  onToggle: (compoundKey: string, dataSource?: string, hasNestedList?: boolean) => void
  dataSource?: string
}
...
const NestedNavItem: React.FC<NestedNavItemProps> = ({
  ...
  const itemDataSource = typeof item.dataSource === 'string' ? item.dataSource : undefined
  const effectiveDataSource = itemDataSource || dataSource
  const isRouteTarget = typeof effectiveDataSource === 'string' && effectiveDataSource.startsWith('/')
  const routeSegments = isRouteTarget ? effectiveDataSource.split('/').filter(Boolean) : []
  const compoundKey = isRouteTarget && routeSegments.length >= 2
  ...
  return (
    <div key={itemKey} className="lateral-bar__nested-item">
      ...
      onClick={() => onToggle(compoundKey, effectiveDataSource, hasNestedList)}
